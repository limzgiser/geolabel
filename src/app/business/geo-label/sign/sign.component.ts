...
import {legentItem, SearchParams, searchTagResult, tagDetailInfo, tagListItem} from "../types";
...
export class SignComponent implements OnInit,OnDestroy {
  ...
  @ViewChild('labelSearchComponent',{static:false}) private labelSearchComponent :LabelSearchComponent;
  ngOnInit() {
    this.mapboxMapService.init().pipe( ).subscribe((mapboxMap: any) => {
  ...
  addGroupLayer() {
    this.mapboxMapService.removeLayerByIds(['cityfun.null.2','cityfun.null.1'])
  ...
  bindMapEvent():void {
    ...
    let self = this;
    offMapEvent(  this.mapboxMap, 'click',"search-tag-layer-click",this.eventCallBack,'search-tag-layer');
    offMapEvent(  this.mapboxMap, 'click',"all-tag-points-my-click",this.eventCallBack,'all-tag-points-my');
    offMapEvent(  this.mapboxMap, 'click',"all-tag-points-other-click",this.eventCallBack,'all-tag-points-other');
    // 阻止点击事件多选要素
  ...
  addMarker(): void {
    ...
    // 鼠标移动
    offMapEvent(  this.mapboxMap, 'mousemove',  "mousemove-addTag",this.eventCallBack);
    // 鼠标点击
    ...
    this.eventCallBack['click-map'] = function (e) {
      ...
      offMapEvent( self.mapboxMap, 'mousemove','mousemove-addTag',  self.eventCallBack
      );
    };
